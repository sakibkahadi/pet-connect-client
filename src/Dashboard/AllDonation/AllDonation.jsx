  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosSecure.delete(`/donationCampaigns/${id}`).then((res) => {
  ...
  return (
    <div>
      {donationCampaigns.length > 0 && (
        <div>
          <MainTitle heading="All Donations Campaign" />
          <div>
                  ...
                  {donationCampaigns.map((campaign, index) => (
                    <tr key={campaign._id}>
                      ...
                      <td className="flex flex-col gap-1 items-center  text-xl">
                        <Link
                          to={`/dashboard/myDonationCampaign/${campaign._id}`}
                        >
                          <FaEdit></FaEdit>
                        </Link>
      ...
      {donationCampaigns.length === 0 && (
        <div>
          <Title subHeading="No Donation Campaign is Added By Users"></Title>
        </div>
      )}
    </div>
  );
