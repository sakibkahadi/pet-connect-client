...
const PetListing = () => {
  ...
  useEffect(() => {
    if (Array.isArray(loadedData)) {
      ...
      const filteredPets = searchData
        ? remaining.filter((pet) =>
            pet.petName.toLowerCase().includes(searchData.toLowerCase())
          )
        : remaining;

      // Category filter
      const categoryFilteredPets = selectedCategory
        ? filteredPets.filter(
            (pet) =>
  ...
  return (
    <div>
      ...
      <div className="flex justify-center gap-6 mb-6">
        ...
        <div className="relative text-gray-600">
          <select
            className="bg-white h-14 px-5 pr-10 rounded-full text-sm focus:outline-none appearance-none"
